...
'''
Reminder: Batch normalization 是一種 regularization
(作者曾在文章中用一段說 dropout 可以不必用了)

但在我們的實驗裡，這個 regulairization 可能太強了
而且奇怪的是: 如果 fully connected layer 也加，竟然會使 network 發散!?
(噢不對，發散是因為 lr=1e-2)
'''
...
from util import full_layer, conv_layer, bn_layer, mse, config
...
class CNN(object):
    ...
    def __init__(self,
        ...
        for i in range(len(n_channel) -1):
            ...
            # inputs_shape = conv_and_filter.get_shape()
            param_shape = conv_and_filter.get_shape()[-1:]
            bn = bn_layer(conv_and_filter, param_shape, 'batchnorm%d' % i,
                axis=[0, 1, 2], 
        ...
        for i, n in enumerate(n_hidden):
            ...
            bn = bn_layer(x, param_shape, 'batchnormfull%d' % i,
                axis=[0],
                is_training=is_training)
            x = bn
